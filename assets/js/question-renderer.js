...
(function (global) {
    ...
    function renderFullQuestion(q, idx, saved) {
        ...
        const contentHtml = stemContent && !isInline ? `<div class="q-stem">${types ? types.escapeHtmlForDisplay(stemContent) : stemContent}</div>` : '';
        const headerHtml = `
            <div class="q-header">
        ...
        const promptHtml = q.prompt && q.prompt.trim()
            ? `<div class="q-prompt-block">${String(q.prompt).replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/\\n/g, '<br>')}</div>`
            : '';
        ...
        if (q.image && String(q.image).trim()) {
            ...
            if (/^data:image\//.test(img)) {
                imageHtml = `<div class="q-image-block" style="max-width:100%;overflow:hidden;"><img src="${img}" alt="Hình minh họa" style="max-width:100%;height:auto;display:block;margin:0 auto;"></div>`;
                ...
                imageHtml = `<div class="q-image-block" style="max-width:100%;overflow:hidden;"><img src="${img}" alt="Hình minh họa" style="max-width:100%;height:auto;display:block;margin:0 auto;" onerror="this.style.display='none'"></div>`;
            }
        }
        // MCQ vẫn render bình thường (có đáp án A/B/C/D)
        const answerHtml = renderQuestion(q, saved);
        return `
